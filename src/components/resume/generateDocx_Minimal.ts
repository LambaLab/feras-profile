...
function sectionHeading(label: string): Paragraph {
  return new Paragraph({
    children: [new TextRun({ text: label, bold: true, size: 18, color: '111111', allCaps: true })],
    border: { bottom: { style: BorderStyle.SINGLE, size: 6, color: 'CCCCCC', space: 4 } },
    spacing: { before: 280, after: 100 },
  })
}

export async function generateDocxBlob_Minimal(profile: Profile): Promise<Blob> {
  ...
  children.push(new Paragraph({
    children: [
      new TextRun({ text: profile.location, size: 18, color: 'AAAAAA' }),
      new TextRun({ text: '  ·  ', size: 18, color: 'AAAAAA' }),
      new TextRun({ text: profile.linkedinUrl.replace('https://', ''), size: 18, color: '111111' }),
      new TextRun({ text: '  ·  ', size: 18, color: 'AAAAAA' }),
      new TextRun({ text: profile.email, size: 18, color: '111111' }),
    ],
    spacing: { after: 200 },
  }))
  ...
  for (const exp of profile.experience) {
    children.push(new Paragraph({ children: [new TextRun({ text: exp.company, bold: true, size: 20 }), new TextRun({ text: `   ${exp.startDate} – ${exp.endDate}`, size: 18, color: 'AAAAAA' })], spacing: { before: 160, after: 40 } }))
    children.push(new Paragraph({ children: [new TextRun({ text: exp.title, italics: true, size: 19, color: '555555' })], spacing: { after: 40 } }))
    children.push(new Paragraph({ children: [new TextRun({ text: exp.location, size: 17, color: 'AAAAAA' })], spacing: { after: 80 } }))
  ...
  for (const edu of profile.education) {
    ...
    children.push(new Paragraph({ children: [new TextRun({ text: `${edu.startYear} – ${edu.endYear}`, size: 17, color: 'AAAAAA' })], spacing: { after: 80 } }))
  }
  ...
  if (profile.certifications.length > 0) {
    ...
    for (const cert of profile.certifications) {
      ...
      children.push(new Paragraph({ children: [new TextRun({ text: cert.issuer, size: 17, color: 'AAAAAA' })], spacing: { after: 60 } }))
    }
  }
